...
def discriminator(images, reuse=False):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        h1 = tf.layers.conv2d(images, filters=64, kernel_size=2, padding='same')
        h1 = tf.maximum(0.2 * h1, h1)
        
        h2 = tf.layers.conv2d(h1, filters=128, kernel_size=2, padding='same')
        h2 = tf.maximum(0.2 * h2, h2)
        
        h3 = tf.layers.conv2d(h2, filters=256, kernel_size=2, padding='same')
...
def generator(z, out_channel_dim, is_train=True):
    ...
    with tf.variable_scope('generator', reuse=not is_train):
        ...
        h2 = tf.layers.conv2d_transpose(h1, filters=128, kernel_size=4, strides=1)
        h2 = tf.layers.batch_normalization(h2, training=is_train)
        h2 = tf.maximum(0.2 * h2, h2)
        # 7x7x128
        
        h3 = tf.layers.conv2d_transpose(h2, filters=64, kernel_size=4, strides=2, padding='same')
        h3 = tf.layers.batch_normalization(h3, training=is_train)
        h3 = tf.maximum(0.2 * h3, h3)
        # 14x14x64
        
        logits = tf.layers.conv2d_transpose(h3, filters=out_channel_dim, kernel_size=4, strides=2, padding='same')
